...
aba = st.sidebar.radio(
    ...
    key="nav_radio_principal",
)

# --- BACKUP E RESTAURAÇÃO NA SIDEBAR ---
st.sidebar.markdown("---")
st.sidebar.subheader("💾 Backup dos Dados (JSON)")
...
if os.path.exists(ARQUIVO_DADOS):
  ...
  st.sidebar.download_button(
      ...
      key="btn_download_sidebar_json",
  )
else:
  st.sidebar.info("Nenhum dado gerado para download ainda.")

arquivo_upload = st.sidebar.file_uploader(
    "Restaurar backup (JSON)", type=["json"], key="uploader_sidebar_json"
)
if arquivo_upload is not None:
  try:
    ...
    if isinstance(dados_carregados, list):
      ...
      st.sidebar.error("Formato JSON inválido (deve ser uma lista).")
...
if aba == "⚙️ Configurações":
  ...
  with col_sem_1:
    novo_total_semanas = st.number_input(
        ...
        key="input_total_semanas_config",
    )
  with col_sem_2:
    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("💾 Salvar Semanas", key="btn_salvar_semanas_config"):
  ...
  with col_cfg1:
    ...
    if st.button("Adicionar Status", key="btn_add_status_config"):
      if (
          novo_status
  ...
  with col_cfg2:
    ...
    if st.button("Adicionar Função", key="btn_add_funcao_config"):
      if (
          nova_funcao
...
# 2. ABA DE GERENCIAR ELENCO E INCLUSÃO DE JORNADA
  ...
  with st.sidebar.form("form_elenco_novo_sidebar", clear_on_submit=True):
    nome_peao = st.text_input("Nome do Participante", key="sidebar_nome_peao")

    lista_status_cadastro = [""] + st.session_state["config_status"]
    status_peao = st.selectbox(
        "Status Inicial", lista_status_cadastro, key="sidebar_status_peao"
    )

    foto_peao = st.text_input(
        "URL da Foto",
        placeholder="https://exemplo.com/foto.jpg",
        key="sidebar_foto_peao",
    )
  ...
  if not participantes:
    ...
    with col_ord1:
      criterio_ordenacao = st.selectbox(
          ...
          label_visibility="collapsed",
          key="select_criterio_ordenacao_elenco",
      )
      ...
      for local_idx, p in enumerate(lote):
        ...
        with cols_lote[local_idx]:
          ...
          if st.button(
              ...
              key=f"btn_sel_grid_{global_idx}",
              use_container_width=True,
          ):
            st.session_state["peao_selecionado"] = p["nome"]
            st.rerun()
        ...
        with col_form:
          ...
          with st.form("form_marco_direto", clear_on_submit=True):
            atualizar_status = st.checkbox(
                "Atualizar status geral do peão?",
                value=False,
                key="chk_atualizar_status_geral",
            )
            ...
            semana_selecionada_str = st.selectbox(
                "Selecione a Semana",
                lista_semanas_opcoes,
                key="sel_semana_marco",
            )

            lista_funcoes_opcoes = [""] + st.session_state["config_funcoes"]
            funcao = st.selectbox(
                "Função / Tarefa (Selo)",
                lista_funcoes_opcoes,
                key="sel_funcao_selo_marco",
            )

            descricao = st.text_area(
                "Descrição do Acontecimento", key="txt_desc_marco"
            )
